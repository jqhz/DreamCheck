...
export default function DreamCheck() {
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col overflow-hidden bg-gray-50 p-4 md:p-8">
      {/* Navbar */}
      <nav className="relative z-20 w-full flex items-center justify-between px-8 py-4 bg-white bg-opacity-90 backdrop-blur border-b border-gray-300 mb-8 rounded-xl shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold py-2 px-5 rounded-lg transition duration-200 text-base"
          >
            Log Out
          </button>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="bg-white bg-opacity-95 rounded-2xl shadow-xl p-8 w-full max-w-4xl min-h-[600px] flex flex-col justify-between gap-10 border border-gray-200 mx-2 md:mx-8">
          ...
          <div className="flex items-center justify-center gap-8 mb-8">
            <button onClick={handlePrevDay} disabled={selectedDate <= creationDate} className="px-4 py-2 bg-indigo-100 hover:bg-indigo-200 rounded-lg font-semibold disabled:opacity-50">Previous</button>
            <input
              type="date"
              value={selectedDate}
              ...
              min={creationDate}
            />
            <button onClick={handleNextDay} disabled={selectedDate >= today} className="px-4 py-2 bg-indigo-100 hover:bg-indigo-200 rounded-lg font-semibold disabled:opacity-50">Next</button>
          </div>
          {/* Main row: Checklist (left), Routine + Dream Journal (right) */}
          <div className="flex flex-1 gap-8 mb-8">
            {/* Checklist (left, tall) */}
            <div className="flex-1 flex flex-col bg-blue-50 rounded-xl p-6 shadow-inner border border-gray-300 gap-6">
              <h3 className="text-xl font-bold text-indigo-700 mb-4">Checklist</h3>
              <ul className="flex-1 overflow-y-auto mb-4 flex flex-col gap-3 px-2">
                {checklist.map((item, idx) => (
                  <li
                    key={idx}
                    /*className={`flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-200 border border-gray-300 gap-3 cursor-pointer select-none*/
                      /*${item.checked ? 'bg-green-300 text-gray-800 line-through' : 'bg-gray-200 text-gray-900'}`}*/
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-lg transition-all duration-200 border border-gray-300 gap-3 cursor-pointer select-none
                      ${item.checked ? 'bg-green-300 text-gray-800 line-through' : 'bg-gray-200 text-gray-900'}`}
                  ...
                  >
                    ...
                    <button className="ml-4 text-xs text-red-500 hover:underline" onClick={e => { e.stopPropagation(); handleRemove(idx); }}>Delete</button>
                  </li>
                ))}
              </ul>
              {/* Checklist Add Item (like Routine) */}
              <div className="flex gap-3 mt-2">
                ...
                />
                <button className="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded-lg font-semibold" onClick={handleAdd}>Add</button>
              </div>
            </div>
            {/* Routine + Dream Journal (right) */}
            <div className="flex-1 flex flex-col gap-6">
              {/* Routine (top right) */}
              <div className="bg-yellow-50 rounded-xl p-6 shadow-inner flex-1 flex flex-col border border-yellow-300 mb-2 gap-6">
                ...
                <ul className="flex-1 overflow-y-auto mb-2 flex flex-col gap-2">
                  {routine.map((item, idx) => (
                    ...
                    >
                      ...
                      {routineEditMode && (
                        <button className="ml-4 text-xs text-red-500 hover:underline" onClick={e => { e.stopPropagation(); handleRoutineRemove(idx); }}>Delete</button>
                      )}
                    </li>
                  ))}
                </ul>
                {routineEditMode && (
                  <div className="flex gap-2 mt-2">
                    ...
                    />
                    <button className="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded" onClick={handleRoutineAdd}>Add</button>
                  </div>
                )}
              </div>
              {/* Dream Journal (bottom right) */}
              <div className="bg-indigo-50 rounded-xl p-6 shadow-inner flex-1 flex flex-col border border-indigo-200 mb-2 gap-4">
                <h3 className="text-xl font-bold text-indigo-700 mb-2">Dream Journal</h3>
                <textarea
                  className="flex-1 border border-gray-300 rounded px-3 py-2 resize-none min-h-[100px]"
                  value={dream}
          ...
          <div className="flex items-center justify-center gap-6 mt-4">
            ...
            <button onClick={handleSave} className="bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-lg font-semibold flex items-center gap-2">
              Save
            </button>
          </div>
        </div>
